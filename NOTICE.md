...
## Scope of this file in this repository

This file is carried over from `MicroWriter-BASIC-PaperS3`, and the
attribution chain it records still applies here: the firmware in
`port-staging/` is that project's code, which is in turn descended from
MicroWriter and MicroSlate, and the fonts in `research/fonts/` are the same
third-party files under the same licences.

Three details differ in this repository and are noted rather than silently
edited, because an attribution file is the wrong place to be approximate:

- Paths that read `editor/` below refer to code that lives in
  `port-staging/` here until it is ported, at which point it moves into
  `editor/`.
- `research/fonts/previews/` was not copied. It held renderings at the
  PaperS3's cell sizes, which do not apply to a 480x320 panel. The `src/`
  files behind them are all here.
- The reference to `editor/LICENSE` below is stale: no such file exists in
  the PaperS3 repository either. The MIT terms it names are reproduced in
  the paragraph that references it, so the attribution itself is intact.
