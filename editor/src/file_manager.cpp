...
#include <Arduino.h>
// Arduino's SD library rather than freeink-sdk's SDCardManager, for the same
// reason tb_runtime.cpp swapped: that wrapper is SdFat inside an e-paper SDK,
// and this board's card is alone on its own bus with nothing to arbitrate. The
// APIs line up; the two that differ are handled at their call sites, open modes
// being strings instead of O_* flags and File::name() returning a full path
// where FsFile::getName() filled a buffer with a bare one.
#include <FS.h>
#include <SD.h>
#include <Utf8.h>
#include <cstring>
...
void deriveUniqueFilename(const char* title, char* out, int maxLen, const char* except) {
  ...
  int suffix = 2;
  while (SD.exists(path) && suffix <= 99) {
...
void fileManagerSetup() {
  // Already mounted by main.cpp before anything got this far; this only reports
  // whether a card is actually present, without the side effect of remounting.
  ...
  for (const CollectionInfo& c : COLLECTIONS) {
    if (!SD.exists(c.dir)) SD.mkdir(c.dir);
  }
  ...
  for (auto file = root.openNextFile(); file; file = root.openNextFile()) {
    {
      ...
      strncpy(name, slash ? slash + 1 : full, sizeof(name) - 1);
...
void loadFile(const char* filename) {
  ...
  // Cast because Arduino's File::read takes uint8_t* where SdFat's took void*.
  // The bytes are the same; only the signature moved.
  int readResult = file.read(reinterpret_cast<uint8_t*>(buf), TEXT_BUFFER_SIZE - 1);
...
void saveCurrentFile(bool refreshList) {
  ...
  auto file = SD.open(tmpPath, FILE_WRITE);
  if (!file) {
  ...
  if (SD.exists(path)) {
    SD.remove(bakPath);          // Remove old .bak (if any)
    SD.rename(path, bakPath);    // Original becomes new .bak
  }
