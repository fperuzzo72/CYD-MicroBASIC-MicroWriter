...
void screenEditorSetMode(int n);  // 0-3, see docs/PORTING_PLAN.md's SCREEN table

// The slice of the panel the terminal owns, in pixels. The row count and the
// centring margin are derived from it, where the two earlier devices store both
// as per-mode constants measured against their own panel. Call once at startup,
// and again if the band ever moves. Shrinking scrolls the content so the cursor
// stays visible.
