...
#include <Arduino.h>
// Arduino's SD library, not freeink-sdk's SDCardManager.
//
// The PaperS3 reaches the card through SDCardManager, which wraps SdFat and
// hands out FsFile handles. That lives in freeink-sdk, an e-paper SDK with a
// panel driver and a power manager attached, and pulling it in for file
// handles would drag all of it. This board's card is on its own VSPI bus with
// nothing to arbitrate, so the plain Arduino SD library is enough.
//
// The two APIs line up almost exactly (open/read/write/close/available/size/
// isDirectory/openNextFile), which is why this is a substitution rather than a
// rewrite. Two places differ and are handled at their call sites: open modes
// are strings here instead of O_* flags, and File has name() where FsFile has
// getName(buf, size).
//
// Paths are unchanged from the PaperS3 on purpose, so a card can be moved
// between the two machines and both find their programs.
#include <FS.h>
#include <SD.h>
...
static File ifile;      // open for reading
static File ofile;      // open for writing
static File rootDir;    // directory walk
static File rootEntry;  // current entry in that walk
...
static void ensureDir() {
  if (!SD.exists("/MicroBASIC")) SD.mkdir("/MicroBASIC");
  if (!SD.exists(TB_DIR)) SD.mkdir(TB_DIR);
}
...
static void reportFileFailure(const char* what, const char* path) {
  ...
  // cardType() rather than a begin(): the card is already mounted by main.cpp
  // and re-running begin() from inside a failure report would be a side effect
  // in a diagnostic.
  snprintf(msg, sizeof(msg), "?card=%d dir=%d", (int)(SD.cardType() != CARD_NONE),
           (int)SD.exists(TB_DIR));
  screenEditorTermPrintLine(msg);
}
...
uint8_t ofileopen(const char* filename, const char* m) {
  ...
  // FILE_WRITE is "w" in this library, which truncates; FILE_APPEND is "a".
  // Both create. That is exactly the O_* pair this replaced.
  ofile = SD.open(path, append ? FILE_APPEND : FILE_WRITE);
...
uint8_t rootnextfile() {
  ...
  // File::name() where FsFile has getName(buf, size). On this core name()
  // returns the full path, so take the part after the last separator: callers
  // expect a bare filename, which is what FILES lists and what LOAD takes.
  {
    const char* full = rootEntry.name();
    const char* slash = strrchr(full, '/');
    strncpy(rootNameBuf, slash ? slash + 1 : full, sizeof(rootNameBuf) - 1);
    rootNameBuf[sizeof(rootNameBuf) - 1] = '\0';
  }
  return 1;
}
