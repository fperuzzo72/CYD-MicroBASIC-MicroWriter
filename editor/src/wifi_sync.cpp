...
static void ensureCollectionDir(const Collection& coll) {
  ...
  if (!SD.exists(coll.dir)) SD.mkdir(coll.dir);
}
...
static void handleFileList() {
  ...
  for (auto file = dir.openNextFile(); file; file = dir.openNextFile()) {
    {
      // File::name() where FsFile::getName() filled a buffer. On this core it
      // returns the full path, so the bare filename is what follows the last
      // separator: the sync protocol sends names, not paths, and a path here
      // would be a name the other end could never match.
...
static bool handleFileDownload() {
  ...
  while (file.available()) {
    int bytesRead = file.read(reinterpret_cast<uint8_t*>(buf), sizeof(buf));
...
static void handleUploadData() {
  ...
  if (up.status == UPLOAD_FILE_START) {
    ...
    upload_.file = SD.open(upload_.path, FILE_WRITE);
